# ファシリテーター用ガイド — GitHub入門講座（初級編・30分）

**受講者は読まなくて構いません。** 進行役・TA向けの運営マニュアルです。
...
その代わり、**事前の招待作業が運営側の最重要タスク**になります（後述）。
...
> ⚠️ `rm -rf` を受講者に打たせる時は、**必ずTAや進行役が手元を見ながら**にしてください。
> パスを間違えると別のものが消えます。
...
受講者が編集**する前**に `git pull` させ、**その後で**進行役が `main` を更新します。
